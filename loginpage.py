...
def media_1():   # Solo disponible para usuarios registrados y +1 peticiones dentro ficha BBDD
    if 'user' in session:      # Hay una sesión iniciada
        ...
        if (respuesta['result'] == 'updated'):
            return render_template('index.html',msj=session['user'] + " está conectado",peticiones='Peticiones de media BBDD local: '+ str(session['peticiones']),media1="Media BBDD local: " + str(media))
    else:
...
def media_2():
    if 'user' in session:      # Hay una sesión iniciada
        ...
        media = media/n_datos
        return render_template('index.html',msj=session['user'] + " está conectado",peticiones='Peticiones de media BBDD local: '+ str(session['peticiones']),media2="Media BBDD internet: " + str(media))
    else:
        return render_template('index.html',msj="Media solo disponible para usuarios registrados")
